<?php
class Hj_Process_Model_Category extends Hj_Process_Model_Process_Abstract
{
    ...
    public function updatePath($entry)
    {
    	...
    	if ($entry['parent_id'] == 0)
    	{
	    	if ($entry['name'] == $entry['path']) {
				...
				Mage::log($parent, null, "a.log", true);
	    		$this->getCategories();
	    		$id = $this->existingCategories[$parent];
				Mage::log($id, null, "z.log", true);
				Mage::log($this->existingCategories, null, "x.log", true);
	    		$select = $category->getCollection()
		                ->getSelect()
		                ->where('category_id = '.$id);
				Mage::log($select, null, "b.log", true);
