<?php 
class Hj_Process_Adminhtml_ProcessGroupController extends Mage_Adminhtml_Controller_Action
{
	...
	{
		$this->_initAction();
		// $this->_addContent($this->getLayout()->createBlock('process/adminhtml_processgroup'));
		$this->renderLayout();
	}
	    ...
	    if(!is_array($requestIds)) {
	        ...
	        try {
	            foreach ($requestIds as $requestId) {
	                $RequestData = Mage::getModel('process/process_group')->load($requestId);                    
	                $RequestData->delete();                    
	            }
	...
	public function deleteAction()
	{
		if( $this->getRequest()->getParam('id') > 0 ) {
			try {
				$processModel = Mage::getModel('process/process_group');
